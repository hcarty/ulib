...
(* $Id: uarray1_with_c_layout.ml,v 1.2 2007-07-13 12:53:27 fclement Exp $ *)
...
let init kind n f =
  let a = Array1.create kind c_layout n in
...
let iter f a =
  let n = length a in
  for i = 0 to n - 1 do f (get a i) done;;

let map f a =
  let n = length a in
  let b = Array1.create (Array1.kind a) c_layout n in
  for i = 0 to n - 1 do set b i (f (get a i)) done;
  b;;

let fold_left f x a =
  let n = length a in
  let r = ref x in
  for i = 0 to n - 1 do r := f !r (get a i) done;
  !r;;
...
let same_length s a1 a2 =
  ...
  invalid_arg ("Uarray1_with_c_layout." ^ s);;

let fold_left2 f x a1 a2 =
  let n = same_length "fold_left2" a1 a2 in
  let r = ref x in
  for i = 0 to n - 1 do r := f !r (get a1 i) (get a2 i) done;
  !r;;
